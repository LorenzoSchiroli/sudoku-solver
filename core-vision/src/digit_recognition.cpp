 ...
 */
std::vector<int>
DigitRecognizer::predictList(const std::vector<cv::Mat> &images) {
 ...
 */
std::vector<int>
DigitRecognizer::predictBatch(const std::vector<cv::Mat> &batchImages) {
  ...
  for (int i = 0; i < batchSize; ++i) {
    ...
    if (i < batchImages.size()) {
      // We assume batchImages only contains valid mats now (filtered by
      // predictList), but checking !empty() is good safety.
  ...
  try {
    ...
    for (size_t i = 0; i < batchImages.size(); ++i) {
      ...
      if (batchImages[i].empty()) {
        ...
        results.push_back(getArgMax(
            std::vector<float>(imgResultStart, imgResultStart + numClasses)));
      }
    }
 ...
 */
bool DigitRecognizer::preprocessToBuffer(const cv::Mat &inputImage,
                                         float *dst) {
  cv::Mat processed;
  ...
  float scale = std::min((float)canvasW / processed.cols,
                         (float)canvasH / processed.rows);
  ...
  cv::Mat blob =
      cv::dnn::blobFromImage(padded, 1.0 / 255.0, cv::Size(canvasW, canvasH),
...
DigitRecognizer::predictFolder(const std::string &folderPath) {
...
DigitRecognizer::predictGrid(const std::vector<std::vector<cv::Mat>> &grid) {
  // 1. Flatten
