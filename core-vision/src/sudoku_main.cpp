 ...
 */
SudokuMain::SudokuMain(const std::string &modelPath) : recognizer(modelPath) {}

/**
 * Process a color image and return the detected/solved Sudoku grid.
 * This function orchestrates detection, grid extraction, recognition and
 * solving.
 */
SudokuResult SudokuMain::sudoku_img2grid(const cv::Mat &originalImage) {
  ...
  // auto t1_ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1_end -
  // t1_start).count(); std::cout << "Step 1 (Sudoku Detection) took: " << t1_ms
  // << " ms" << std::endl;
  ...
  // Returns 9x9 grid of cell images (empty cells are handled internally by your
  // function)
  std::vector<std::vector<cv::Mat>> cellCrops =
      analyze_sudoku_board(*boardCrop);
  // auto t2_end = std::chrono::high_resolution_clock::now();
  // auto t2_ms = std::chrono::duration_cast<std::chrono::milliseconds>(t2_end -
  // t2_start).count(); std::cout << "Step 2 (Grid Extraction) took: " << t2_ms
  // << " ms" << std::endl;
  ...
  // auto t3_ms = std::chrono::duration_cast<std::chrono::milliseconds>(t3_end -
  // t3_start).count(); std::cout << "Step 3 (Digit Recognition) took: " <<
  // t3_ms << " ms" << std::endl;
  ...
  // auto t4_ms = std::chrono::duration_cast<std::chrono::milliseconds>(t4_end -
  // t4_start).count(); std::cout << "Step 4 (Sudoku Solving) took: " << t4_ms
  // << " ms" << std::endl;

  // If solved, update numbers in the single grid; preserve mask so original
  // detections are still marked.
  if (success) {
    for (int i = 0; i < 9; ++i) {
      for (int j = 0; j < 9; ++j) {
